...
- 工作流运行状态和岗位产物以本地文件保存，主要写入 `workspaces/runs/<run_id>/`。
...
核心流水线由赵爽（热点监控）、星辰（爆款分析）、洛一（文案）、小李（剪辑）和尤道理（运营）组成。扩展岗位包括方舟（产品经理）、阿栈（程序员）、林量（股票助手）和周周（心理疗愈）。
